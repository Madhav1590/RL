...
class SequenceDataset(IterableDataset):
    def __init__(
        ...
        self.dataset, info = load_d4rl_trajectories(env_name, episode_len=episode_len, gamma=1.0)
    ...
    def __prepare_sample(self, traj_idx, n):
        ...
        for j in idx_sorted:
            ret.append(pad_along_axis(calculate_new_returns(R_0, rewards[j]), pad_to=self.episode_len))
            rew.append(pad_along_axis(rewards[j], pad_to=self.episode_len))
            d[j, dataset_len[j] - 1] = 1 if returns[j] >= R_0 else 0

            mask.append(np.hstack([np.zeros(dataset_len[j], dtype=np.float32),
                                   np.full(self.episode_len - dataset_len[j], fill_value=float("-inf"), dtype=np.float32, ),
                                  ]))

        states = np.asarray(states, dtype=np.float32)[idx_sorted].reshape(n * self.episode_len, -1)
        actions = np.asarray(actions, dtype=np.float32)[idx_sorted].reshape(n * self.episode_len, -1)
        rewards = np.asarray(rew, dtype=np.float32).reshape(n * self.episode_len, -1)  # already in the right order
        returns = np.asarray(ret, dtype=np.float32).reshape(n * self.episode_len, -1)  # already in the right order
        ...
        mask = pad_along_axis(mask, pad_to=(self.n_traj_max * self.episode_len), fill_value=float("-inf"))
        ...
        return (states, actions, returns, rewards, d, time_steps, mask, ), best_traj_range
